 ...
 * Title : AC
 * description : 선영이는 주말에 할 일이 없어서 새로운 언어 AC를 만들었다.
...
public class P5430 {
	...
	static void solution(ArrayDeque<Integer> deque, char[] cmd){
		...
		for(char c : cmd){
			if(c=='R'){
				...
				if(change) deque.removeLast();
				else deque.removeFirst();
			}
		}
		Iterator<Integer> iterator = (change ? deque.descendingIterator() : deque.iterator());
